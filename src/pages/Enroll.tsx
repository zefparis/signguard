...
type IdentityFormProps = {
  ...
  onIdNumberChange: (e: ChangeEvent<HTMLInputElement>) => void
...
const IdentityForm = memo(function IdentityForm({
  ...
  return (
    <>
      <div className="badge badge-purple">Step 1 of 6 — Identity</div>
      <h1 className="step-title">Identity Certification</h1>
      <p className="step-sub">This creates a legally certifiable signer identity profile.</p>
      <form onSubmit={onSubmit} style={{ width: '100%' }}>
        ...
        <div className="field">
          <label>ID Number (national ID / passport) *</label>
          <input value={form.idNumber} onChange={onIdNumberChange} required placeholder="ID123456" />
        </div>
        <div className="field">
          <label>Organization / Company *</label>
          <input value={form.organization} onChange={onOrganizationChange} required placeholder="Acme Legal" />
        </div>
        <div className="field">
          <label>Role / Title *</label>
          <input value={form.role} onChange={onRoleChange} required placeholder="General Counsel" />
        </div>
        <div className="field">
...
export function Enroll() {
  ...
  const handleIdNumberChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleIdentity = useCallback((e: FormEvent) => {
    e.preventDefault()
    if (!form.firstName || !form.lastName || !form.idNumber || !form.organization || !form.role) return
    setStep('selfie')
  ...
  async function handleReaction(ms: number) {
    ...
    try {
      ...
      const res = await enrollWorker({
        ...
        email: form.email || `${form.firstName}.${form.lastName}@signguard.local`,
        tenant_id: import.meta.env.VITE_TENANT_ID,
        cognitive_baseline: payloadBaseline,
      })
      setSignerId(res.student_id)
      setConf(Math.round(res.confidence))
      setSigner({
        signerId: res.student_id,
  ...
  return (
    <BehavioralCapture enabled={step !== 'identity'} onController={onBehavioralController}>
      <div className="page">
        ...
        {step === 'selfie' && (
        <>
          <div className="badge badge-purple">Step 2 of 6 — Biometric</div>
          <h1 className="step-title">Face Certification</h1>
        ...
        {step === 'submitting' && (
        <>
          <h1 className="step-title">Certifying...</h1>
          <p className="step-sub">Creating your biometric identity profile</p>
        ...
        {step === 'success' && (
        <>
          <div className="badge badge-green" style={{ margin: '0 auto 20px' }}>✓ Identity Certified</div>
          <h1 className="step-title">Identity Certified</h1>
          <p className="step-sub">You can now sign documents with a post-quantum certificate.</p>
          ...
          <div className="card" style={{ width: '100%', marginTop: 8 }}>
            <div className="metric-row">
              <span className="metric-label">Signer ID</span>
              <span className="metric-value" style={{ fontSize: 11 }}>{signerId.slice(0,12)}...</span>
            </div>
            <div className="metric-row">
            ...
            <div className="metric-row">
              ...
              <span className="metric-value">{form.organization || '—'}</span>
            </div>
            <div className="metric-row">
              <span className="metric-label">Role</span>
              <span className="metric-value">{form.role || '—'}</span>
            </div>
            <div className="metric-row">
          ...
          <button className="btn btn-primary" style={{ marginTop: 20 }} onClick={() => nav('/sign')}>
            Sign a Document →
          </button>
        </>
      )}
