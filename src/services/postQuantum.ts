...
// Deterministic signature for an arbitrary SHA3-256 hex string (32 bytes)
export function signDocumentHash(documentHashHex: string, privateKeyHex: string): string {
  const sk = hexToBytes(privateKeyHex)
  const pk = ml_kem768.getPublicKey(sk)
  const msg = hexToBytes(documentHashHex)
  if (msg.length !== 32) throw new Error('document_hash must be SHA3-256 (32 bytes hex)')
  const { cipherText } = ml_kem768.encapsulate(pk, msg)
  return bytesToHex(cipherText)
}

export function verifyDocumentHash(documentHashHex: string, signatureHex: string, publicKeyHex: string): boolean {
  try {
    const pk = hexToBytes(publicKeyHex)
    const msg = hexToBytes(documentHashHex)
